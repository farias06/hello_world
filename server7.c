...
main (void)
{
  ...
  putGeneric2 (MYPATH_CA, client_accepted, vdebug, vsyslog);
  putGeneric2 (MYPATH_CR, client_refused, vdebug, vsyslog);
  ...
  while (1)
      ...
      for (i = 0; i < MAXCLIENTS; i++)
	{
      ...
      putGeneric2 (MYPATH_LOAD, nb, vdebug, vsyslog);
      ...
      if (select (highest + 1, &readfds, NULL, NULL, &tv) >= 0)
	{
	  if (FD_ISSET (sockfd, &readfds))
		...
		{
		                          client_refused++;
                        putGeneric2 (MYPATH_CA, client_refused, vdebug, vsyslog);
		  send (new_fd, "\nTOO MANY CLIENT\n", 17, MSG_NOSIGNAL);
		  close (new_fd);
		}
	    }
